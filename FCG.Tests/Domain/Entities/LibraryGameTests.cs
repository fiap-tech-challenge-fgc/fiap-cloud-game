...
public class LibraryGameTests
{
 ...
 [Fact]
   ...
   var game = new Game("7891234560001", "Test Game", "Action", "Description");
        ...
        // Act
        var libraryGame = new LibraryGame(game, player, purchasePrice);

        // Assert
        Assert.NotEqual(Guid.Empty, libraryGame.Id);
        Assert.Equal(game.Id, libraryGame.GameId);
        Assert.Equal(game, libraryGame.Game);
  ...
  [Fact]
    public void Constructor_DeveLancarExcecao_QuandoPrecoNegativo()
    {
      ...
      var player = new Player(Guid.NewGuid(), "TestPlayer");
        ...
        // Act & Assert
      var exception = Assert.Throws<ArgumentException>(() => new LibraryGame(game, player, precoInvalido));
  ...
  // Arrange
      ...
      // Act
    var libraryGame = new LibraryGame(game, player, precoZero);
   ...
   // Act
  var libraryGame = new LibraryGame(game, player, 59.99m);
...
decimal price = 99.99m;
        ...
        // Act
        var libraryGame = new LibraryGame(game, player, price);

        // Assert
        Assert.Equal(game.Id, libraryGame.GameId);
     Assert.Equal(player.Id, libraryGame.PlayerId);
      Assert.Equal(price, libraryGame.PurchasePrice);
      Assert.NotNull(libraryGame.Game);
 ...
 public void PurchaseDate_DeveSerImmutable()
    {
        ...
        var game = new Game("7891234560001", "Test Game", "Action", "Description");
        var player = new Player(Guid.NewGuid(), "TestPlayer");
        var libraryGame = new LibraryGame(game, player, 49.99m);
 ...
 var player = new Player(Guid.NewGuid(), "GamerPro");
        var libraryGame = new LibraryGame(game, player, 79.90m);
...
var player = new Player(Guid.NewGuid(), "RPGFan");
        var libraryGame = new LibraryGame(game, player, 199.90m);
  ...
  // Arrange
   ...
   var player2 = new Player(Guid.NewGuid(), "Player2");
        ...
        // Act
  var library1Game1 = new LibraryGame(game1, player1, 49.99m);
  var library1Game2 = new LibraryGame(game2, player1, 59.99m);
        var library2Game1 = new LibraryGame(game1, player2, 39.99m); // Mesmo jogo, preço diferente
...
Assert.Equal(player1.Id, library1Game2.PlayerId);
    ...
    Assert.Equal(game1.Id, library1Game1.GameId);
        Assert.Equal(game1.Id, library2Game1.GameId); // Mesmo jogo
    }
 ...
 decimal promotionalPrice = 49.99m; // 50% off
        ...
        // Act
        var libraryGame = new LibraryGame(game, player, promotionalPrice);
  ...
  var player1 = new Player(Guid.NewGuid(), "EarlyBuyer");
      ...
      // Act - Player 1 compra no lançamento
        var earlyPurchase = new LibraryGame(game, player1, 199.90m);
  
        System.Threading.Thread.Sleep(100); // Simula tempo passando
        
  // Player 2 compra em promoção
        var latePurchase = new LibraryGame(game, player2, 99.90m);
   ...
   // Assert
        ...
        Assert.Equal(game.Id, earlyPurchase.GameId);
        Assert.Equal(game.Id, latePurchase.GameId);
    }
 ...
 new Game("7891234560002", "Game 2", "RPG", "Desc 2"),
   ...
   System.Threading.Thread.Sleep(10); // Pequeno delay entre compras
            libraryGames.Add(new LibraryGame(games[i], player, prices[i]));
        }
 ...
 // Arrange
    ...
    var player = new Player(Guid.NewGuid(), "FreebieCollector");
        ...
        // Act
        var libraryGame = new LibraryGame(game, player, 0m);
 ...
 decimal premiumPrice = 9999.99m;
        ...
        // Act
      var libraryGame = new LibraryGame(game, player, premiumPrice);
  ...
  [Fact]
    public void LibraryGame_DeveManterReferenciaParaGameEPlayer()
    {
     ...
     var player = new Player(Guid.NewGuid(), "TestPlayer");
        ...
        // Assert
        Assert.Same(game, libraryGame.Game); // Mesma referência
        Assert.Same(player, libraryGame.Player); // Mesma referência
        Assert.Equal(game.Title, libraryGame.Game.Title);
