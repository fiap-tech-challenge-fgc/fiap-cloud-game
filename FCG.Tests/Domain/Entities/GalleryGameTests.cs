...
/// <summary>
/// Testes unitários para a entidade GalleryGame
/// </summary>
...
// Assert
  ...
  Assert.Equal(price, galleryGame.FinalPrice); // Sem promoção, preço final = preço original
...
// Arrange
 ...
 DateTime.UtcNow.AddDays(-1) // Expirou ontem
  ...
  var promotion = Promotion.Create(
            PromotionType.FixedDiscount,
            100m, // Desconto maior que o preço
       DateTime.UtcNow.AddDays(-1),
            DateTime.UtcNow.AddDays(1)
        );
   ...
   // Act
        ...
        // Assert
        Assert.True(galleryGame.FinalPrice >= 0); // Não deve ser negativo
    }
...
DateTime.UtcNow,
      DateTime.UtcNow.AddDays(7)
        ...
        // Assert
    Assert.Contains("70,00", description); // Preço com desconto
    }
  ...
  var game = new Game("7891234560001", "Awesome Game", "RPG", "Epic adventure");
     var galleryGame = new GalleryGame(game, 150m);
        ...
        // Act & Assert - Sem promoção
     Assert.Equal(150m, galleryGame.FinalPrice);

        // Aplica promoção de 20%
        var promotion1 = Promotion.Create(
    PromotionType.PercentageDiscount,
        20m,
      DateTime.UtcNow,
   ...
   // Substitui por promoção de R$ 50
        var promotion2 = Promotion.Create(
            PromotionType.FixedDiscount,
     50m,
   DateTime.UtcNow,
