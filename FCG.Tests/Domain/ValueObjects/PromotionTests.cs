...
/// <summary>
/// Testes unitários para o Value Object Promotion
/// </summary>
public class PromotionTests
{
 ...
 Assert.Equal(tipo, promotion.Type);
  ...
  var valor = 50m;
        var inicio = DateTime.UtcNow;
        var fim = DateTime.UtcNow.AddDays(-7); // Fim antes do início

        // Act & Assert
        var exception = Assert.Throws<ArgumentException>(() => 
        Promotion.Create(tipo, valor, inicio, fim));
    Assert.Contains("Data de fim deve ser posterior à data de início", exception.Message);
    }
...
[Fact]
    ...
    // Arrange
     var tipo = PromotionType.None;
        var valor = 0m;
        var inicio = DateTime.UtcNow;
        var fim = DateTime.UtcNow.AddDays(-1); // Fim antes do início, mas tipo é None
 ...
 // Arrange
  ...
  // Act
    ...
    {
        // Arrange - Valor negativo pode ser permitido para representar acréscimo
        var tipo = PromotionType.FixedDiscount;
      var valor = -10m;
        var inicio = DateTime.UtcNow;
        var fim = DateTime.UtcNow.AddDays(7);
 ...
 Assert.Equal(PromotionType.None, promotion.Type);
  ...
  [Fact]
    ...
    [Fact]
    public void IsActive_DeveRetornarTrue_QuandoDataDentroDoPeríodo()
    {
        // Arrange
    ...
    public void ApplyDiscount_DeveRetornarPrecoOriginal_QuandoDescontoFixoMaiorQuePreco()
{
      ...
      var promotion = Promotion.Create(
 PromotionType.FixedDiscount,
    150m, // Desconto maior que o preço
          DateTime.UtcNow.AddDays(-1),
         DateTime.UtcNow.AddDays(1)
        );
      var precoOriginal = 100m;
...
var promotion = Promotion.Create(
   ...
   // Act
     ...
     // Assert
    Assert.Equal(100m, precoFinal); // Metade do preço
    }
...
// Arrange
 ...
 // Act & Assert
  ...
  var precoOriginal = 100m;
        ...
        // Act
     var isActiveBeforeExpiry = promotion.IsActive(inicio.AddDays(2)); // Meio do período
   ...
   var priceAfterExpiry = promotion.ApplyDiscount(precoOriginal);
        ...
        Assert.Equal(precoOriginal, priceAfterExpiry); // Sem desconto após expirar
    }
